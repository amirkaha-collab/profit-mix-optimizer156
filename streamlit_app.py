...
.tb{display:flex;align-items:center;justify-content:space-between;
  padding:0 32px;height:52px;background:#0f172a;
  border-bottom:1px solid rgba(255,255,255,.06)}
.tb-left{display:flex;align-items:center;gap:10px}
.tb-mark{width:28px;height:28px;border-radius:7px;
  background:linear-gradient(135deg,#1e3a6e,#2563eb);
  display:flex;align-items:center;justify-content:center;
  font-size:14px;flex-shrink:0}
.tb-names{display:flex;flex-direction:column;gap:1px}
.tb-co{font-size:13.5px;font-weight:800;color:#f1f5f9;letter-spacing:-.3px;line-height:1.1}
.tb-prod{font-size:9.5px;font-weight:500;color:rgba(96,165,250,.6);letter-spacing:.2px}
.tb-right{display:flex;align-items:center;gap:8px}
.tb-beta{border:1px solid rgba(250,204,21,.2);border-radius:999px;
  padding:2px 8px;font-size:8.5px;font-weight:700;
  color:rgba(251,191,36,.55);letter-spacing:1px;text-transform:uppercase}

/* PAGE TITLE */
.pt{padding:36px 0 10px;direction:rtl;text-align:right}
.pt-h{font-size:34px;font-weight:900;color:#0f172a;letter-spacing:-1.4px;margin:0 0 9px}
...
/* CARD */
.card{background:#fff;border-radius:18px;padding:28px 24px 24px;
  position:relative;overflow:hidden;
  box-shadow:0 1px 8px rgba(15,23,42,.06),0 1px 2px rgba(15,23,42,.04);
  border:1px solid #edf2f7;direction:rtl;text-align:right;
  display:flex;flex-direction:column;height:100%;
  transition:transform .2s,box-shadow .2s,border-color .2s}
.card:hover{transform:translateY(-3px);
  box-shadow:0 12px 32px rgba(15,23,42,.1),0 3px 6px rgba(15,23,42,.05);
  border-color:#dbeafe}
.card-primary{border-color:#dbeafe;background:linear-gradient(160deg,#f8fbff 0%,#fff 60%)}
.card-top-rule{position:absolute;top:0;left:0;right:0;height:4px;
  background:var(--rule,#e2e8f0);border-radius:20px 20px 0 0}
.card-badge{display:inline-flex;align-items:center;gap:5px;
  ...
  color:var(--badge,#64748b);margin-bottom:18px;opacity:.85}
.card-ico{width:50px;height:50px;border-radius:14px;
  display:flex;align-items:center;justify-content:center;
  font-size:24px;line-height:1;margin-bottom:18px;
  background:var(--ico-bg,#f8fafc);
  box-shadow:0 2px 8px rgba(0,0,0,.07)}
.card-title{font-size:20px;font-weight:900;color:#0f172a;
  letter-spacing:-.5px;margin:0 0 10px;line-height:1.2}
.card-desc{font-size:13px;color:#475569;line-height:1.72;
  margin:0 0 auto;flex-grow:1}
.card-btn{display:block;width:100%;margin-top:24px;padding:12px 0;
  border-radius:11px;font-size:13.5px;font-weight:700;
  text-align:center;cursor:pointer;letter-spacing:-.1px;
  background:#0f172a;color:#fff;border:none;
  transition:background .15s,transform .1s}
.card-btn:hover{background:#1e293b;transform:translateY(-1px)}
.card-btn-outline{background:#fff;color:#0f172a;
  border:2px solid #e2e8f0}
.card-btn-outline:hover{background:#f8fafc;border-color:#cbd5e1}

/* QUICK LINKS */
.ql{display:flex;gap:10px;flex-wrap:wrap;direction:rtl;
  padding:6px 0;justify-content:flex-end}
.ql-item{display:inline-flex;align-items:center;gap:6px;
  font-size:12.5px;font-weight:600;color:#475569;cursor:pointer;
  padding:7px 16px;border-radius:999px;border:1.5px solid #e2e8f0;
  background:#fff;transition:color .15s,border-color .15s,background .15s,box-shadow .15s}
.ql-item:hover{color:#1d4ed8;border-color:#93c5fd;background:#eff6ff;
  box-shadow:0 2px 8px rgba(37,99,235,.1)}

/* PRODUCT-WORLD SELECTORS inside cards */
.pw-label{font-size:11px;font-weight:700;color:#475569;direction:rtl;
  margin-bottom:8px;letter-spacing:.2px}
.pw-row{display:flex;gap:6px;flex-wrap:wrap;direction:rtl;margin-bottom:16px}
.pw-btn{display:inline-flex;align-items:center;padding:5px 11px;
...
.pw-btn:hover,.pw-btn.active{background:#eff6ff;border-color:#3b82f6;color:#1d4ed8}

/* DISCLAIMER */
.disc{background:#fffceb;border:1px solid #fde68a;
  border-right:3px solid #f59e0b;border-radius:10px;
  padding:9px 16px;direction:rtl;text-align:right;
...
.disc-item{font-size:9.5px;color:#78350f;padding:2px 9px;
  border-radius:999px;background:rgba(245,158,11,.08);
...
.gd-tip{background:#ebf2ff;border-right:4px solid #2563eb;
  border-radius:0 10px 10px 0;padding:10px 14px;
  font-size:12px;color:#1e3a8a;margin-top:14px;line-height:1.55}

/* SECTION DIVIDER */
.sdiv{border:none;height:1px;background:#f1f5f9;margin:24px 0 0}
</style>
"""
...
def render_landing_page(df_long: "pd.DataFrame"):
    """Clean minimal one-screen homepage. Ends with st.stop()."""
    st.markdown(_LANDING_CSS, unsafe_allow_html=True)
    ...
    st.markdown("""
<div class="pt">
  <div class="pt-h">בחר איך תרצה לעבוד</div>
  <p class="pt-s">תוכל לבחור עולם מוצר בתוך כל מסלול</p>
</div>""", unsafe_allow_html=True)
...
<div class="card card-primary"
     style="--rule:#3b82f6;--badge:#2563eb;--ico-bg:#eff6ff">
  <div class="card-top-rule"></div>
  <div class="card-badge">&#x25B6; מסלול ראשי</div>
  <div class="card-ico">&#x1F4CA;</div>
  <div class="card-title">ניתוח תיק לקוח</div>
  <div class="card-desc">קבל תמונה מלאה על התיק שלך לפני קבלת החלטות</div>
</div>""", unsafe_allow_html=True)
        if st.button("התחל ניתוח", key="lp_c1", use_container_width=True, type="primary"):
            _nav_to("app", "תיק לקוח")
            st.rerun()

    with c2:
        _opt_worlds = ["קרנות השתלמות","קופות גמל","פוליסות חיסכון","קרנות פנסיה"]
        if "lp_opt_sel" not in st.session_state:
            st.session_state["lp_opt_sel"] = None
        _opt_sel = st.session_state["lp_opt_sel"]

        # build world-button HTML for display
        _opt_btns = "".join(
            f"<span class=\"pw-btn{'  active' if w == _opt_sel else ''}\">{w}</span>"
            for w in _opt_worlds
        )
        st.markdown(f"""
<div class="card" style="--rule:#475569;--badge:#475569;--ico-bg:#f8fafc">
  <div class="card-top-rule"></div>
  <div class="card-ico">&#x1F3AF;</div>
  <div class="card-title">אופטימיזציית תמהיל</div>
  <div class="card-desc">בנה תמהיל השקעה מותאם לפי יעדים ומגבלות</div>
  <div class="pw-label">בחר עולם מוצר:</div>
  <div class="pw-row">{_opt_btns}</div>
</div>""", unsafe_allow_html=True)
        # Actual clickable world selectors
        _ow_cols = st.columns(len(_opt_worlds))
        for _i, (_oc, _ow) in enumerate(zip(_ow_cols, _opt_worlds)):
            with _oc:
                _short = _ow.replace("קרנות ","").replace("קופות ","")
                if st.button(_short, key=f"lp_ow_{_i}",
                             use_container_width=True,
                             type="primary" if _opt_sel==_ow else "secondary"):
                    st.session_state["lp_opt_sel"] = _ow
                    st.rerun()
        _btn_disabled = (_opt_sel is None)
        if st.button(
            "התחל אופטימיזציה" if not _btn_disabled else "⬆ בחר עולם מוצר תחילה",
            key="lp_c2", use_container_width=True,
            type="primary" if not _btn_disabled else "secondary",
            disabled=_btn_disabled,
        ):
            _nav_to("app", _opt_sel)
            st.rerun()

    with c3:
        _isa_worlds = ["קרנות השתלמות","קופות גמל","פוליסות חיסכון","קרנות פנסיה"]
        if "lp_isa_sel" not in st.session_state:
            st.session_state["lp_isa_sel"] = None
        _isa_sel = st.session_state["lp_isa_sel"]

        _isa_btns = "".join(
            f"<span class=\"pw-btn{'  active' if w == _isa_sel else ''}\">{w}</span>"
            for w in _isa_worlds
        )
        st.markdown(f"""
<div class="card" style="--rule:#475569;--badge:#475569;--ico-bg:#f8fafc">
  <div class="card-top-rule"></div>
  <div class="card-ico">&#x1F3E6;</div>
  <div class="card-title">ניתוח אסטרטגיות מוסדיים</div>
  <div class="card-desc">השווה בין גופים ומסלולים ולמד מה השתנה לאורך זמן</div>
  <div class="pw-label">בחר עולם מוצר:</div>
  <div class="pw-row">{_isa_btns}</div>
</div>""", unsafe_allow_html=True)
        _iw_cols = st.columns(len(_isa_worlds))
        for _i, (_ic, _iw) in enumerate(zip(_iw_cols, _isa_worlds)):
            with _ic:
                _short = _iw.replace("קרנות ","").replace("קופות ","")
                if st.button(_short, key=f"lp_iw_{_i}",
                             use_container_width=True,
                             type="primary" if _isa_sel==_iw else "secondary"):
                    st.session_state["lp_isa_sel"] = _iw
                    st.rerun()
        _isa_disabled = (_isa_sel is None)
        if st.button(
            "השווה אסטרטגיות" if not _isa_disabled else "⬆ בחר עולם מוצר תחילה",
            key="lp_c3", use_container_width=True,
            type="primary" if not _isa_disabled else "secondary",
            disabled=_isa_disabled,
        ):
            _nav_to("app", _isa_sel)
            st.rerun()
    ...
    st.markdown("""
<div class="ql">
  <span class="ql-item">&#x1F4C4; הפק דוח</span>
  <span class="ql-item">&#x1F916; שאל את ה-AI</span>
  <span class="ql-item">&#x1F4D0; למד מהמוסדיים</span>
  <span class="ql-item">&#x1F680; התחל מתיק קיים</span>
</div>""", unsafe_allow_html=True)
